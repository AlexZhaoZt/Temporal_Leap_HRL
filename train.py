...
def collect_experience(tf_env, agent, meta_agent, state_preprocess,
  ...
  def leap():
    ...
    tf_env._env.envs[0]._gym_env.t += agent.tf_context.meta_action_every_n
      # increment time step 
    context_t_inc_op = tf.assign_add(agent.tf_context.t, agent.tf_context.meta_action_every_n)
      
    increment_op = [increment_step_op, increment_ep_op, context_t_inc_op]

    with tf.control_dependencies(increment_op):

      goal_state = meta_action + state_repr
      goal_state = tf.concat([goal_state, state[15:]], 0)
      pos = goal_state[:2]
      def temp_fn(pos):
        tf_env._env.envs[0]._gym_env.wrapped_env.set_xy(pos)
        return tf.constant(0.0)

      set_xy_op = tf.py_func(temp_fn, [pos], [tf.float32])
      low_level_context = meta_action_fn(goal_state, context=None)

      #agent_context_upd = [tf.assign(var, value)
      #           for var, value in zip(agent.context_vars, low_level_context)]

      state_var_upd = tf.assign(state_var, goal_state)
      meta_action_var_upd = tf.assign(meta_action_var, low_level_context)
    
    #return tf.group(set_xy_op, state_var_upd, meta_action_var_upd, *agent_context_upd)
    return tf.group(set_xy_op, state_var_upd, meta_action_var_upd)
  ...
  threshold = 0.9 ** -(environment_steps / 300000)
  rand = np.random.uniform()
  return tf.cond(rand > threshold, leap, walk)
  #if rand > threshold:
  #  return leap()
  #else:
  #  return walk()
...
def train_uvf(train_dir,
  ...
  for mode in ['meta', 'nometa']:
    ...
    with tf.name_scope(mode):
      ...
      train_vars_crit = agent.get_trainable_critic_vars()
      critic_train_op = slim.learning.create_train_op(
          ...
          summarize_gradients=summarize_gradients,
          clip_gradient_norm=clip_gradient_norm,
          variables_to_train=train_vars_crit,)
